...
from src.models.DCVC_copy import DCVC_net
...
def encode_one(args_dict, device):
    ...
    video_net_dict = video_net.state_dict()
    shared_dict = {k: v for k, v in load_checkpoint.items() if k in video_net_dict}
    video_net_dict.update(shared_dict)   
    video_net.load_state_dict(video_net_dict, strict=True)
