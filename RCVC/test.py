        ...
        for frame_idx in range(frame_num):
            ...
            if args_dict['write_stream']:
                ...
                if frame_idx % gop_size == 0:
                    ...
                    bits_con.append(0)
                    bits_feature_res.append(0)
                    bits_z_res.append(0)
                    bits_res.append(0)
                    
                else:
                    result = video_net.encode_decode(ori_frame,ref_frame, bin_path)
                    ref_frame_con = result['recon_image']
                    bpp = result['bpp']
                    frame_types.append(1)
                    bits_con.append(bpp*frame_pixel_num)
                    ...
                    ref_frame = result['recon_image_all']       
                    bpp_all = result['bpp_all']
                    bits.append(bpp_all*frame_pixel_num)        
                    bits_feature_res.append(result['bpp_feature_res']*frame_pixel_num)
                    bits_z_res.append(result['bpp_z_res']*frame_pixel_num)
                    bits_res.append(result['bpp_res']*frame_pixel_num)
            else:
                if frame_idx % gop_size == 0:
                    ...
                    result = video_net(ori_frame,ref_frame)
                    ref_frame_con = result['recon_image']
                    ref_frame_res = result['recon_image_res']
                    ref_frame = 0.5 * ref_frame_con + 0.5 * ref_frame_res
                    
                    mse_all = torch.mean((ref_frame - ori_frame).pow(2))
                    mse_con = torch.mean((ref_frame_con - ori_frame).pow(2))
                    mse_res = torch.mean((ref_frame_res - ori_frame).pow(2))
                    print("MSE_con",mse_con)
                    print("MSE_res",mse_res)
                    print("MSE",mse_all)
 
                    bpp = result['bpp']
                    frame_types.append(1)
                    bits_con.append(bpp.item()*frame_pixel_num)
...
#                     ref_frame = result['recon_image_all']   
                    bpp_all = result['bpp_all']
                    bits.append(bpp_all.item()*frame_pixel_num) 
                    bits_feature_res.append(result['bpp_feature_res'].item()*frame_pixel_num)          
                    bits_z_res.append(result['bpp_z_res'].item()*frame_pixel_num)
                    bits_res.append(result['bpp_res'].item()*frame_pixel_num)
    ...
    cur_all_p_frame_bit_all = 0
    cur_all_p_frame_bit_res = 0
    cur_all_p_frame_bit_feature_res = 0
    cur_all_p_frame_bit_z_res = 0
    ...
    for idx in range(frame_num):
        if frame_types[idx] == 0:
            ...
            cur_all_p_frame_bit_con += bits_con[idx]
            ...
            cur_all_p_frame_bit_all += bits[idx]
            cur_all_p_frame_bit_res += bits_res[idx]
            cur_all_p_frame_bit_feature_res += bits_feature_res[idx]
            cur_all_p_frame_bit_z_res += bits_z_res[idx]
    ...
    if cur_p_frame_num > 0:
        ...
        log_result['ave_p_frame_bpp_all'] = cur_all_p_frame_bit_all / total_p_pixel_num
        
        log_result['ave_p_frame_bpp_con'] = cur_all_p_frame_bit_con / total_p_pixel_num
        ...
        log_result['ave_p_frame_bpp_res'] = cur_all_p_frame_bit_res / total_p_pixel_num
        log_result['ave_p_frame_bpp_feature_res'] = cur_all_p_frame_bit_feature_res / total_p_pixel_num
        log_result['ave_p_frame_bpp_z_res'] = cur_all_p_frame_bit_z_res / total_p_pixel_num
    else:
        log_result['ave_p_frame_bpp_all'] = 0
        
        log_result['ave_p_frame_bpp_con'] = 0
        ...
        log_result['ave_p_frame_bpp_res'] = 0
        log_result['ave_p_frame_bpp_feature_res'] = 0
        log_result['ave_p_frame_bpp_z_res'] = 0
        
    log_result['ave_all_frame_bpp'] = (cur_all_i_frame_bit + cur_all_p_frame_bit_all) / \
...
def filter_dict(result):
    keys = ['i_frame_num', 'p_frame_num', 'ave_i_frame_bpp', 'ave_i_frame_quality', 'ave_p_frame_bpp_all','ave_p_frame_bpp_con',
            'ave_p_frame_bpp_mv_y', 'ave_p_frame_bpp_mv_z', 'ave_p_frame_bpp_y',
            'ave_p_frame_bpp_z','ave_p_frame_bpp_res','ave_p_frame_bpp_feature_res', 'ave_p_frame_bpp_z_res','ave_p_frame_quality','ave_all_frame_bpp','ave_all_frame_quality']
...
def main():
    ...
    for ds_name in config:
        for seq_name in config[ds_name]['sequences']:#对应的是一个字典，len()=7，也就是循环7次
            count_sequences += 1
            for model_idx in range(len(args.model_path)):#循环四次，对应4个模型
                cur_dict = {}
                cur_dict['model_idx'] = model_idx #0/1/2/3
                ...
                cur_dict['video_path'] = seq_name #'video_path': 'Beauty_1920x1024_120fps_420_8bit_YUV'
                cur_dict['gop'] = config[ds_name]['sequences'][seq_name]['gop']
                cur_dict['frame_num'] = config[ds_name]['sequences'][seq_name]['frames']
                cur_dict['dataset_path'] = config[ds_name]['base_path'] #'dataset_path': '/mnt/DCVC/media/data/UVGDataSet_crop'
                ...
                cur_dict['ds_name'] = ds_name #'ds_name': 'UVG'
#                 print("dic",cur_dict)
    ...
    for ds_name in config:
        ...
        for seq in config[ds_name]['sequences']:
            ...
            for model_idx in range(len(args.model_path)):
                ckpt = os.path.basename(args.model_path[model_idx]) #返回path最后的文件名             
                for res in results:
    ...
    final_result = {} 
    for ds_name in config:
        final_result[ds_name] = {}        
        for model_idx in range(len(args.model_path)):   
            ckpt = os.path.basename(args.model_path[model_idx])
            final_result[ds_name][ckpt] = {}
            final_result[ds_name][ckpt]["U_ave_all_frame_bpp"] = 0########################
            final_result[ds_name][ckpt]["U_ave_all_frame_quality"] = 0#######################                        
    ...
    for ds_name in config:   
        for seq in config[ds_name]['sequences']:
            for model_idx in range(len(args.model_path)): 
                ckpt = os.path.basename(args.model_path[model_idx])
                final_result[ds_name][ckpt]["U_ave_all_frame_bpp"] += log_result[ds_name][seq][ckpt]["ave_all_frame_bpp"] 
                final_result[ds_name][ckpt]["U_ave_all_frame_quality"] += log_result[ds_name][seq][ckpt]["ave_all_frame_quality"] 
#                 print("ckpt",ckpt)
#                 print("PSNR",final_result[ds_name][ckpt]["U_ave_all_frame_quality"])

    for model_idx in range(len(args.model_path)): 
        ckpt = os.path.basename(args.model_path[model_idx])
        final_result[ds_name][ckpt]["U_ave_all_frame_bpp"] = final_result[ds_name][ckpt]["U_ave_all_frame_bpp"]/7 
        final_result[ds_name][ckpt]["U_ave_all_frame_quality"] = final_result[ds_name][ckpt]["U_ave_all_frame_quality"]/7
